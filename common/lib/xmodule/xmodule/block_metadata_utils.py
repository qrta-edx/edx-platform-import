...
def url_name_for_block(block):
    """
    Given a block's usage locator, returns the block's URL name.

    Arguments:
        location (BlockUsageLocator): The block's usage locator.
    """
...
def display_name_with_default_escaped(block):
    ...
    return block.display_name_with_default.replace('<', '&lt;').replace('>', '&gt;')
